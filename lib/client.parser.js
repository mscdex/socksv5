...
var ERRORS = {
      GENFAIL: ['general SOCKS server failure', 'EGENFAIL'],
      DISALLOW: ['connection not allowed by ruleset', 'EACCES'],
      NETUNREACH: ['network is unreachable', 'ENETUNREACH'],
      HOSTUNREACH: ['host is unreachable', 'EHOSTUNREACH'],
      CONNREFUSED: ['connection refused', 'ECONNREFUSED'],
      TTLEXPIRED: ['ttl expired', 'ETTLEXPIRED'],
      CMDUNSUPP: ['command not supported', 'ECMDNOSUPPORT'],
      ATYPUNSUPP: ['address type not supported', 'EATYPNOSUPPORT']
    },
    ...
    switch (state) {
      ...
      */
      case STATE_VERSION:
        if (chunk[i] !== 0x05) {
          this.emit('error',
                    new Error('Unsupported SOCKS protocol version: ' + chunk[i]));
          return;
        }
        ++i;
        if (this.authed)

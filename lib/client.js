...
function Client(options) {
  ...
  this._auths = [];
  this._localDNS = (typeof options.localDNS === 'boolean'
                    ? options.localDNS
                    : true);
  this._strictLocalDNS = (typeof options.strictLocalDNS === 'boolean'
...
Client.prototype.connect = function(options, cb) {
  ...
  if (net.isIP(this._dstaddr) === 0 && this._localDNS) {
    dns.lookup(this._dstaddr, function(err, addr) {
      ...
      if (addr)
        self._dstaddr = addr;
      self._sock.connect(self._proxyport, self._proxyhost);
    });
  } else
...
exports.connect = exports.createConnection = function() {
  var args = normalizeConnectArgs(arguments);
  return Client.prototype.connect.apply(new Client(args[0]), args);
};
